...
* **OV   (TCGA Ovarian Cancer)**:
    ...
    \item \textbf{PAAD (TCGA Pancreatic Cancer)}: Pancreatic cancer begins in the tissues of your pancreas. The pancreas produces enzymes that help digestion and makes hormones that help manage blood sugar.
    \item \textbf{PCPG (TCGA Pheochromocytoma \& Paraganglioma)}: These are rare tumors that come from the same type of tissue. Paraganglioma starts in nerve tissues in the adrenal glands and near certain blood vessels and nerves. Paragangliomas that form in the adrenal glands are called pheochromocytomas.
    \item \textbf{PRAD (TCGA Prostate Cancer)}: Prostate cancer is a type of cancer that occurs in the prostate. This is one of the most common cancers among men.
    \item \textbf{READ (TCGA Rectal Cancer)}: This cancer occurs in the rectum that is the last part of the large intestine.
    \item \textbf{SARC (TCGA Sarcoma)}: Soft tissue sarcoma is a rare type of cancer that occurs in the tissues that connect, support and surround other body structures. This can contain nerves, muscle, blood vessels, fat, tendons and the lining of the joints.
    \item \textbf{SKCM (TCGA Melanoma)}: Melanoma, the most serious type of skin cancer, develops in the cells called melanocytes that make melanin which is the pigment gives the skin its color.
    \item \textbf{STAD (TCGA Stomach Cancer)}: Stomach cancer usually begins in the mucus-producing cells that line the stomach.
    \item \textbf{TCGT (TCGA Testicular Cancer)}: Testicular cancer occurs in the testicles. The testicles release male sex hormones and sperm for reproduction.
    \item \textbf{THCA (TCGA Thyroid Cancer)}: Thyroid cancer occurs in the cells of the thyroid. The thyroid makes hormones that regulate blood pressure, heart rate, body temperature, and weight.
    \item \textbf{THYM (TCGA Thymoma)}: Thymoma and thymic carcinoma are diseases in which cancer cells form on the outside surface of the thymus. The thymus is an organ in the neck that makes T-cells for the immune system. 
    \item \textbf{UCEC (TCGA Endometrioid Cancer)}: Endometrial cancer occurs in the cells that form the inner layer of the uterus (endometrium) and is sometimes called uterine cancer. Other types of cancer can form in the uterus, including uterine sarcoma, but they are much less common than endometrial cancer.
    
    \item \textbf{UCS (TCGA Uterine Carcinosarcoma)}:  This type of cancer starts in the inner layer of the tissue lining the uterus, while sarcoma begins in the outer layer of muscle of the uterus.
    
    \item \textbf{UVM (TCGA Ocular melanomas)}: 
